...
model = "waveletflow"
stepsPerResolution = [2, 2, 8, 8, 8, 8, 8, 8, 8, 16, 16]
stepsPerResolution_L = [None] * 10
nLevels = 9    # Number of DWT levels (log_2(nx))
kernel = 3
baseLevel = 3    # Base level for the DWT, baseLevel=1 means that the first DWT level 2x2 pixels
partialLevel = -1 
hiddenChannels = 256
n_res_blocks = 3
actNormScale = 1.0
...
LU = True
#prior
gauss_priors = [4,5,6,7,8] #set this for HCC prior. For WN prior, add all the layers to this list
priorType = 'CC' #Choose 'CC' or 'C'
unnormalize_prior = [None] + [False]*8 + [True]
normalize_prior = [None] + [False]*2 + [True] + [False]*7
#datset configs 
...
normalize = [None] + [False]*2 + [True] + [False]*7
norm_type = ['std']*10
double_precision = [None, True, True, True, True, True, False, False, False, False]

#powerspectra and normalization
std_path = 'norm_stds/512x512_final_mean_stats_all_levels_kappa_noise_10.0_noise_freq_21600_kappa_cib.json'
ps_path = 'ps/512x512_kappa_noise_10.0_noise_freq_21600_kappa_cib/dwtlevel'

#output paths
saveDir = '/sdf/group/kipac/users/mati/real_hcc_model_512_noised_kappa_nyquest_more_noise_base3/'
plotSaveDir = 'plots_hcc_model_512_noised_kappa_nyquest_more_noise_base_3/'